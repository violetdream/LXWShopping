...
import com.personal.order.dal.persistence.StockMapper;
...
public class SubStockHandler extends AbstractTransHandler {
	...
	public boolean handle(TransHandlerContext context) {
		...
		if(list==null||list.isEmpty()){
			throw new ValidateException("库存未初始化");
		}
		if(list.size()!=itemIds.size()){
			throw new ValidateException("有商品未初始化库存,请在如下商品id中检查库存状态："+itemIds.toString());
			...
			cartProductDtoList.forEach(one -> {
				if (Objects.equals(one.getProductId(), stock.getItemId())) {
					if (stock.getStockCount() < one.getProductNum()) {
						throw new ValidateException(stock.getItemId()+"库存不足");
					}

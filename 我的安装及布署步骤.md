...
flush privileges;
2、更改加密规则
ALTER USER 'root'@'localhost' IDENTIFIED BY '123A456b!' PASSWORD EXPIRE NEVER;
3、更新root用户密码
ALTER USER 'root'@'%' IDENTIFIED WITH mysql_native_password BY '123A456b!';
##这步很重要，不执行，退出后本地无法登录了
ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY '123A456b!';
4、刷新权限
flush privileges;
```
...
修改bind 172.18.138.75 启用protect-mode yes
